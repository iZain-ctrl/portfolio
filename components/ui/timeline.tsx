...
export const TimelineItem: FC<TimelineItemProps> = ({
  ...
  return (
    <div className="flex flex-wrap gap-12 min-h justify-start relative">
      ...
      >
        <p className="text-secondry-foreground">{date}</p>
      </div>
      {/*Right side*/}
      <div 
        className="flex gap-x-2"
        style={{ transform: `${isCourse ? "translateX(-45)" : ""}` }}
      >
...
const Body:FC<BodyProps> = ({link,subTitle,tag}) => {
  return (
    <div className="text-secondry-foreground flex items-center">
      <p className="text-sm font-normal leading-6 mt-1">{subTitle}</p>
      {link ? <FiArrowUpRight /> : null}
      {tag ? (
        <div className="ms-2 rounded-[20^x] bg-white/5 py-0.5 px-1.5">
          <p className="text-[10px] font-normal  text-secondry-foreground">
            {tag}
          </p>
        </div>
      ) : null}
